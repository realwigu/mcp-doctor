...
export function printDoctorSummary(results: ScanResult[], issues: SecurityIssue[]): void {
  const ok = results.filter((r) => r.status === "ok").length;
  const total = results.length;
  ...
  console.log(`  Servers:    ${chalk.cyan(String(total))} found, ${chalk.green(String(ok))} healthy`);
  console.log(`  Security:   ${issues.length === 0 ? chalk.green("0 issues") : chalk.yellow(`${issues.length} issue(s)`)}`);
  if (responding.length > 0) {
    console.log(`  Avg latency: ${chalk.cyan(`${avgLatency}ms`)}`);
  }

  if (ok === total && issues.length === 0) {
    console.log(chalk.green("\n  All clear — your MCP setup looks healthy.\n"));
  } else {
    const problems: string[] = [];
    if (ok < total) problems.push(`${total - ok} server(s) not responding`);
    if (issues.length > 0) problems.push(`${issues.length} security issue(s)`);
